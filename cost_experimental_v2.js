...
//exec(5, 1000, 100, 10, 9999999999999, 100);
...
async function exec(k, n, n_test, n_client, epsilon, total_incentive) {
    ...
    let counter_benchmark = [];
    for (let l = 0; l < n_client; l++) {
        counter_benchmark[l] = 0;
        all_cost_benchmark[l] = [];
    }
    for (let l = 0; l < n_client; l++) {
        for (let l_current = 0; l_current < n_client; l_current++) {
            if (l !== l_current) {
                all_cost_benchmark[l_current][counter_benchmark[l_current]] = await calc_lr_cost(client[l_current].b, client[l].data_train);
                counter_benchmark[l_current]++;
            }
        }
    }
    ...
    //calculate cost 2 | LOO_small_data_test
    ...
    let all_cost_standardized_loos_data_test = await standardize(all_cost_loos_data_test);
    ...
    //calculate cost 4 | central test data set
    ...
    let all_cost_standardized_central = await standardize(all_cost_central);
    ...
    let EuDist_central = await EuclidDistance(all_cost_median_standardized_benchmark, all_cost_standardized_central, 'benchmark-central');
    ...
    let corr_benchmark_central = corr.calc(all_cost_median_standardized_benchmark, all_cost_standardized_central);
    ...
    return [
        [
            ...
            all_cost_standardized_central,
            all_cost_standardized_random
        ],
        [EuDist_private, EuDist_loos_test, EuDist_lool, EuDist_central, EuDist_random],
        [
            corr_benchmark_private,
            corr_benchmark_LOOsmall_test,
            corr_benchmark_LOOlarge,
            corr_benchmark_central,
