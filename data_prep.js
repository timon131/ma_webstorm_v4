...
async function prepare_housing(csvFilePath, k, n, l) {
    const df = await importCsvToArray(csvFilePath);
    //console.log(csvArray);
    const xy = await prepare_df(df, k, n, l);
    return xy;
}
...
async function prepare_df(df, k, n, l) {
    let df_out = [];
...
// todo: must be updated to k+1 before next usage
